...
VALUES (:name, :avatar, :description, :status)";
    ...
    public function deleteProducts($id) {
        ...
        $is_delete = $obj_delete_product->execute();
        return $is_delete;
    }
    ...
    public function getAllPagination($params = [], $str_search)
    {
        $limit = $params['limit'];
        $page = $params['page'];
        $start = ($page - 1) * $limit;
        $obj_select = $this->connection
            ->prepare("SELECT * FROM categories WHERE TRUE $str_search LIMIT $start, $limit");
